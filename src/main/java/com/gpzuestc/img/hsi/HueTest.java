...
public class HueTest {
	...
	public void testLimit(){ //白色饱和度s:0
		...
		System.out.println(new HueRecognizer().computerHue(rgb).getName());
	}
...
//		BufferedImage bufferedImage = ImageIO.read(new URL("http://ww2.sinaimg.cn/bmiddle/7b38fb67jw1e03zfduiphj.jpg"));  //green
		...
		HueRecognizer hueRecognizer = new HueRecognizer();
		List<Hue> ignores = new ArrayList<Hue>();
//		ignores.add(Hue.GREEN);
//		ignores.add(Hue.YELLOW);
//		ignores.add(hueRecognizer.GRAY);
		hueRecognizer.setIgnores(ignores);
//		Hue hue = hueRecognizer.computerHue(bufferedImage, HueRecognizer.AREA_TYPE_1X3, 2);
//		Hue hue = hueRecognizer.computerHue(bufferedImage, HueRecognizer.AREA_TYPE_3X1, 1);
		Hue hue = hueRecognizer.computerHue(bufferedImage, 3, 2, 3);
		System.out.println("get hue:" + (System.currentTimeMillis() - begin));
		System.out.println(hue.getName());
